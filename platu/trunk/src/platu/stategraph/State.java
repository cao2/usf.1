...
public class State extends PlatuObj {
	...
	/* Process where this state belongs. */
	private LPN lpnModel = null;
	
	/* Value computed by hashCode() for hash tables */
	private int hashVal = 0;
		
	/* Each state has an unique index */
	private int index;
	
	/* List of enabled actions and the corresponding successor states. Dynamic */
	private LinkedList<Pair<LPNTran, State>> enbTranTable;
	
	/*	Set of external transitions. Dynamic */
	private HashSet<StateTran> extTranTable;
	...
	public State(State other) {
		...
		this.enbTranTable = other.enbTranTable;
		this.extTranTable = other.extTranTable;
	}
	...
	public void setEnabled(LinkedList<Pair<LPNTran, State>> enabled) {
		this.enbTranTable = enabled;
	}
	
	public LinkedList<Pair<LPNTran, State>> getEnabled() {
		return this.enbTranTable;
	}

	
	public boolean addExtTran(StateTran extTran) {
		if(this.extTranTable == null) {
			this.extTranTable = new HashSet<StateTran>();
		}
	
		State curExtPst = extTran.getCurrentState();
		
		/* need to check the consistency of their shared variables */
		//???
	...
	public HashSet<StateTran> getExtTrans() {

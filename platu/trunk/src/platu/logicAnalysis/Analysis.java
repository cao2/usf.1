	...
	public Pair<Integer, Integer> synchronize(ProcSg psg1, ProcSg psg2) {
		...
		while (waitList.size() > 0) {
			...
			for (Pair<LPNTran, ProcState> tran2 : enabled2) {
				LPNTran act2 = tran2.getLeft(); 
				ProcState succ2 = tran2.getRight();
				
				if (succ2 == ProcSg.badPst) continue;

				ProcStateTran pstTran2 = psg2.getPsTran(pst2, act2, succ2); 							
			...
			for (ProcStateTran tran1 : extTrans1) {
				...
				for (ProcStateTran tran2 : extTrans2) {
					if(tran1 == tran2) {
						System.out.println("extTran is the same");

						System.out.println(tran1);
						System.out.println(tran2);	
						break;
					}
					else if(tran1.equals(tran2)) {
						System.out.println("extTran is equal");
				...
				if (visitedTbl.contains(succPair) == false) {
					...
					int gidx = pst1.getGlobalVecIdx();
					String gvecStr = this.printGlobalVec(this.gVecTable.toIntArray(gidx));
					System.out.println("Global vector = " + gvecStr);
					System.out.println("pst1.local = " + pst1.getLocalState());
					System.out.println("pst2.local = " + pst2.getLocalState() + "\n");	
					
					int gidx_succ = succ1.getGlobalVecIdx();
					String gvecStr_succ = this.printGlobalVec(this.gVecTable.toIntArray(gidx_succ));
					System.out.println("Global vector = " + gvecStr_succ);
					System.out.println("succ1.local = " + succ1.getLocalState());
					System.out.println("succ2.local = " + succ2.getLocalState() + "\n");	
					
					System.out.println(gidx + "  " + gidx_succ + "\n");
}

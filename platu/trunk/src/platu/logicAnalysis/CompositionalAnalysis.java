 ...
 */
public class CompositionalAnalysis extends Analysis {
...
//		}
	 ...
	 */
	public static Pair<Boolean, Boolean> localSearch(StateGraph sg1, StateGraph sg2){
		boolean sg1_changed = false;
		boolean sg2_changed = false;
...
//			}
			...
			if(ext1 != null){
				for(StateTran stateTran : ext1){
					LPNTran enabledTran = stateTran.getAction();
					
					if(synchronousTrans.contains(enabledTran)){
						boolean enabled = false;
						if(ext2 != null){
							for(StateTran stTran : ext2){
								if(stTran.getAction() == enabledTran){
									enabled = true;
									break;
								}
							}
			...
			for(LPNTran enTran : modify){
				...
				if (sg1.addStateTranCompositional(state1, enTran, nextState1) != null) 
					sg1_changed = true;
				if(sg2.addStateTranCompositional(state2, enTran, nextState2) != null)
					sg2_changed = true;
...
//						System.out.println("      (" + state1.getIndex() + ", " + state2.getIndex() + ") -> (" + nextState1.getIndex() + ", " + nextState2.getIndex() + ")");
			...
			for(StateTran enStateTran : synch){
				LPNTran enTran = enStateTran.getAction();
...
//								System.out.println("    NON COMPATIBLE CURRENT STATES");
		...
		return new Pair<Boolean, Boolean>(sg1_changed, sg2_changed);
	}
	 ...
	 * @param sgArray - Set of state graphs to construct
	 */
	public static void craLocalSearch(StateGraph[] sgArray){
		 ...
		 */
		List<Pair<StateGraph,StateGraph>> syncPairArray = new ArrayList<Pair<StateGraph, StateGraph>>();
...
//					}
				...
				Pair<StateGraph,StateGraph> sgPair = new Pair<StateGraph,StateGraph>(sg,sg2);
				if (sg.getIndex() > sg2.getIndex())
					sgPair = new Pair<StateGraph,StateGraph>(sg2,sg);
				
				if (syncPairArray.contains(sgPair)==false)
					syncPairArray.add(sgPair);
			}
		...
		boolean[] sgStatus = new boolean[sgArray.length];
		for (StateGraph sg : sgArray)
			sgStatus[sg.getIndex()] = true;
			
		// Keeps track of the current iteration
		int iter = 0;
		boolean fixpoint = false;
		while (syncPairArray.size() > 0){
			Pair<StateGraph, StateGraph> sgPair = syncPairArray.remove(0);
			StateGraph sg1 = sgPair.getLeft();
			StateGraph sg2 = sgPair.getRight();
			

					String lsinfo = "--> searching " + sg1.getLpn().getLabel() + " and " + sg2.getLpn().getLabel() + "\n";
					lsinfo += "\t" + sg1.getLabel() + ": (" + sg1.reachSize() + ", " + sg1.numTransitions() + ")\t"+ sg2.getLabel() + ": (" + sg2.reachSize() + ", " + sg2.numTransitions() +");";
					Console.print(lsinfo, Console.MessageType.dynamicInfo, 1);
					
			Pair<Boolean,Boolean> newStatus = localSearch(sg1, sg2);
			boolean sg1new = newStatus.getLeft();
			boolean sg2new= newStatus.getRight();
			
			if (sg1new==true) {
				for(StateGraph sg4 : inputSrcMap.get(sg1)) {
					if (sg4 == sg2) continue;
					Pair<StateGraph,StateGraph> sgPairNew = new Pair<StateGraph,StateGraph>(sg1,sg4);
					if (sg1.getIndex() > sg4.getIndex())
						sgPairNew = new Pair<StateGraph,StateGraph>(sg4,sg1);
					
					if (syncPairArray.contains(sgPairNew)==false) {
						syncPairArray.add(sgPairNew);
					}
				}
			}
			
			if (sg2new==true) {
				for(StateGraph sg4 : inputSrcMap.get(sg2)) {
					if (sg4 == sg1) continue;
					Pair<StateGraph,StateGraph> sgPairNew = new Pair<StateGraph,StateGraph>(sg2,sg4);
					if (sg2.getIndex() > sg4.getIndex())
						sgPairNew = new Pair<StateGraph,StateGraph>(sg4,sg2);
...
//			 */
//			boolean[] sgStatusNew = new boolean[sgArray.length];
//			for (StateGraph sg : sgArray)
//				sgStatusNew[sg.getIndex()] = true;
//			
//			for(StateGraph sg1 : sgArray){
//				for(StateGraph sg2 : inputSrcMap.get(sg1)){
//					if (sgStatus[sg1.getIndex()]==true || sgStatus[sg2.getIndex()]==true) {
//
//					String lsinfo = "--> searching " + sg1.getLpn().getLabel() + " and " + sg2.getLpn().getLabel() + "\n";
//					lsinfo += "\t" + sg1.getLabel() + ": (" + sg1.reachSize() + ", " + sg1.numTransitions() + ")\t"+ sg2.getLabel() + ": (" + sg2.reachSize() + ", " + sg2.numTransitions() +");";
//					Console.print(lsinfo, Console.MessageType.dynamicInfo, 1);
//					
//						Pair<Boolean,Boolean> newStatus = localSearch(sg1, sg2);
//						if (newStatus.getLeft()==true || newStatus.getRight()==true)
//							fixpoint = false;
//						if (newStatus.getLeft() == true)
//							sgStatusNew[sg1.getIndex()] = true;
//						if (newStatus.getRight() == true)
//							sgStatusNew[sg2.getIndex()] = true;
//						
//						String tmp = "\t after --> " + sg1.getLabel() + ": (" + sg1.reachSize() + ", " + sg1.numTransitions() + ")\t"+ sg2.getLabel() + ": (" + sg2.reachSize() + ", " + sg2.numTransitions() +");";
//						Console.print(tmp, Console.MessageType.dynamicInfo, 1);
//						System.out.println(newStatus.getLeft() + "  " + newStatus.getRight());
//					}
////					System.out.println(numNewStateTrans);
//				}
//			}
//			sgStatus = sgStatusNew;
//			
//			System.out.println("\n");
//		}
    ...
    public static int buildModuleSG(StateGraph sg, final State baseState){
        ...
        while (!stateStack.empty()){
            ...
            for(int i = 0; i < arrayIndex; i++){
            	...
            	LPNTran firedTran = stTran.getAction();

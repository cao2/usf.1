...
lpn[Project prj] returns [Set<LPN> lpnSet]
    :   	{$lpnSet = new HashSet<LPN>();}
        globalConstants? globalVariables?
        	{
    	...
    	'"' '>' constants? variables instantiation? logic? '<' '/' 'mod' '>'
			{
				...
				$lpn = new LPN(prj, $ID.text, globals, Internals, VarNodeMap, $logic.lpnTranSet, 
	    			...
	    			if(!GlobalInterfaceMap.containsKey($ID.text)){
	    				...
	    				if(value != null){
	    					...
	    					if(internals.contains($ID.text)){
	    						...
	    						outputLPN.getGlobals().add($ID.text);
	    						VarNode outputNode = outputLPN.getVarNode($ID.text);

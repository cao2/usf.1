...
public class LPN extends Common {
    ...
    protected LPNTranSet transitions;
    protected Object initState;
    private Markings initMark;
    ...
    private HashMap<State, LinkedList<LPNTran>> enabledSetTbl;
...
//        ret += getInitStateTimed() + "\n";
    ...
    public LPN(Project prj, String label, VarSet inputs, VarSet outputs,
        ...
        enabledSetTbl = new HashMap<State, LinkedList<LPNTran>>();
    ...
    public LPN clone() {
        ...
        // copy transitions
        LPNTranSet transitions = this.transitions.copy(varNodeMap);
...
//    }
     ...
     */
    public LPNTranSet getTransitions() {
        return transitions;
    }
    
    public LPNTranSet getOutputTrans() {
        LPNTranSet outputTranSet = new LPNTranSet();
...
//    	if(initState == null){
    		StateVector sv = new StateVector(this.initVector);
    		this.initState = new TimedState(this.initMark, sv.toIntArray(this.varIndexMap), this.initZone);
...
//    	}
     ...
     */
    public void setTransitions(LPNTranSet transitions) {
        this.transitions = transitions;
    }
     ...
     */
    public LPNTranSet getEnabled(int stateIdx) {
    	State curState = this.index2StateTbl.get(stateIdx);
        return this.getEnabled(curState);
    }

    // Return the set of all LPN transitions that are enabled in 'state'.
    public LPNTranSet getEnabled(State curState) {
    	if (curState == null) {
            throw new NullPointerException();
        }
    	
    	if(enabledSetTbl.containsKey(curState) == true){
            return (LPNTranSet)enabledSetTbl.get(curState).clone();
        }
    	
        LPNTranSet curEnabled = new LPNTranSet();

...
//        this.hashVal = other.hashVal;
    ...
    public StateVector getOutVector(VarSet outputs, DualHashMap<String, Integer> VarIndexMap) {
    	StateVector outVec = new StateVector();
    	for(int i = 0; i < vector.length; i++) {
     ...
     * @return
     */
    public State update(StateVector newVector, DualHashMap<String, Integer> VarIndexMap) {
    	...
    	for(int index = 0; index < vector.length; index++) {
    		...
    		if(newVector.contains(var) == true) {
    			int newVal = newVector.getValue(var);
    			if(this_val != newVal) {
    				newState = true;
    ...
    public void update(HashMap<String, Integer> varValMap, DualHashMap<String, Integer> VarIndexMap) {
    	HashSet<String> inputSet = lpnModel.getInputs();
    	for(String inputVar : inputSet) {
    		if(varValMap.containsKey(inputVar) == true) {
    			this.vector[VarIndexMap.getValue(inputVar)] = varValMap.get(inputVar);
    		}
    	}
    }

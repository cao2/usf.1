...
public class StateGraph extends AbstractStateGraph {
...
////    }
    ...
    public void printStates(){
    	LPN lpn = (LPN) this;
    	System.out.println(String.format("%-8s    %5s", lpn.getLabel(), "|States| = " + reachableSet.size()));
    }
...
//    }
    ...
    public int constrFindSG(final State baseState){
        ...
        Stack<State> stStack = new Stack<State>();
        Stack<LPNTranSet> tranStack = new Stack<LPNTranSet>();
        LPNTranSet currentEnabledTransitions = getEnabled(baseState);
        ...
        while (true){
            ...
            List<LPNTran> stickyTrans = new LinkedList<LPNTran>();
            
            for(LPNTran lpnTran : currentEnabledTransitions){
            	if(lpnTran.getSticky()){
            		stickyTrans.add(lpnTran);
            	}
            }
...
//            	StateTran stTran = new StateTran(currentState, firedTran, state);
            	...
            	LPNTranSet nextEnabledTransitions = getEnabled(nextState);
...
//        System.out.println(previousEnabledSet.size());
    	for(LPNTran stickyTran : previousEnabledSet){
    		if(!stickyTran.getSticky()) continue;
...
//                System.out.println("\n firedTran : " + firedTran.getFullLabel());
            	for(LPNTran stickyTran : currentEnabledTransitions){
            		if(stickyTran == firedTran || !stickyTran.getSticky()) continue;

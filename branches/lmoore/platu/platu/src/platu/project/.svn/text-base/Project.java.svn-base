...
public class Project extends Common{
	...
	public Project(Set<String> inputs, String Label, Set<String> outputs,//
			Set<String> internals, VarValSet PropSet, LPNTranSet LpnTranSet, //
	...
	public void search() {	
		...
		for (int index = 0; index < lpnCnt; index++) {
			...
			initStateArray[index] = curLpn.add(initStateArray[index]);
		}		
		

		if (Options.getTimingAnalysis()=="on") {
			new TimingAnalysis(modArray); 
			return;
		}
		else if(Options.getTimingAnalysis()=="off") {

...
public class LPNTran extends Common {
    ...
    private static final int LpnTran_START = -1;
    private static final int LpnTran_END = -2;
    private static final String DUMMY_LpnTran = "$";
    
    public LPNTranSet mSet = new LPNTranSet();
...
//        return timerMaxVal == null ? true : this.delayLB <= timerMaxVal;
    ...
    public State[] fire(final LPN[] curLpnArray, final int[] curStateIdxArray, int curLpnIndex) {
    	...
    	for(int i = 0; i < curLpnArray.length; i++)
    		stateArray[i] = curLpnArray[i].get(curStateIdxArray[i]);
    ...
    public State[] fire(final LPN[] curLpnArray, final State[] curStateArray, int curLpnIndex) {
        ...
        if(this.local()==true) {
    		nextStateArray[curLpnIndex] = curLpnArray[curLpnIndex].add(nextState);
        	return nextStateArray;
		}

        int[] nextVector = nextState.getVector();
        StateVector vvSet = new StateVector();
        ...
        for(int i = 0; i < stateLength; i++) {
        	...
        	if (curStateArray[i] != nextStateArray[i]) {
        		nextStateArray[i] = curLpnArray[i].add(nextStateArray[i]);
        	}
        }
    ...
    public State fire(final State curState) {  		
        ...
        State newState = this.lpn.add(new State(this.lpn, curNewMarking, newVectorArray));

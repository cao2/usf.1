...
//    			VarCountMap.put($arg.getText(), 0);
    	...
    	))* )? '"' '>' constants? variables logic '<' '/class' '>'
			{
				...
				$lpn = StateGraph.TIMED_ANALYSIS ? 
					new TimedStateGraph(prj, $modName.text, Inputs, Outputs,
	         			Internals, VarNodeMap, $logic.lpnTranSet, StatevectorMap, $logic.initMarking, zone):
	         		new StateGraph(prj, $modName.text, Inputs, Outputs, Internals, VarNodeMap, $logic.lpnTranSet, 
	         			StatevectorMap, $logic.initMarking, zone);
...
logic returns [Markings initMarking, LPNTranSet lpnTranSet]

...
public class AmpleSubset {
			                                                           ...
			                                                           LPNTranRelation lpnTranRelation)
   {
	 ...
	 * @return
	 */
	public LPNTranSet generateAmpleList(boolean stateVisited,
										String approach,
										LinkedList<LPNTran>[] enabledArray,
										HashMap<LPNTran,HashSet<LPNTran>> allIndepSet)
	{
		LPNTranSet ampleList = new LPNTranSet();
		LPNTranSet enableTranSet = this.getEnableTranSet(enabledArray);
		...
		else
		{
	    	...
	    	{
	    		LPNTranSet interleavingEnabledSet = this.getInterleavingEnabledSet(enableTranSet, allInterleavingTrans);
	 ...
	 * @return
	 */
	private LPNTranSet getInterleavingEnabledSet(LPNTranSet enableTranSet, HashSet<LPNTran> interleavingSet)
	{
		LPNTranSet interleavingEnabledSet = new LPNTranSet();
...
*/
	 ...
	 * @return
	 */
	private LPNTranSet getSubset_noInterleaving(LinkedList<LPNTran>[] enabledList,
			                                    HashMap<LPNTran,HashSet<LPNTran>> indepTranSet)
	{
		LPNTranSet set1 = new LPNTranSet();
		LPNTranSet set2 = new LPNTranSet();
		//order
 ...
 * @return
 */
	private LPNTranSet getSubset_withInterleaving(LPNTranSet interleavingEnabledSet,
			                                      LinkedList<LPNTran>[] enabledList,
			                                      HashMap<LPNTran,HashSet<LPNTran>> indepTranSet,
			                                      HashMap<LPNTran, HashSet<LPNTran>> interleavingSet)
	{
		LPNTranSet subset = new LPNTranSet();
		LPNTranSet dep_interleaving = new LPNTranSet();
		LPNTranSet set1 = new LPNTranSet();
		LPNTranSet set2 = new LPNTranSet();
		//order
		if(enabledList.length>1)
	...
	private LPNTranSet subset_indepIsEmpty(LPNTranSet interleavingEnabledSet,
									       HashMap<LPNTran, HashSet<LPNTran>> interleavingSet,
									       LPNTranSet dep_interleaving,
										   HashMap<LPNTran,HashSet<LPNTran>> allIndepSet)
	{
		LPNTranSet ready_interleaving = new LPNTranSet();
		...
		if(!ready_interleaving.isEmpty())
		{
			if(!dep_interleaving.isEmpty())
			{
				LPNTranSet dep = this.getDepOfInterleavingTrans(ready_interleaving, dep_interleaving, allIndepSet);
		...
		{
			LPNTranSet subset = this.getSubsetOfInterleavingTrans(interleavingEnabledSet);
			if(!dep_interleaving.isEmpty())
			{
				LPNTranSet dep = this.getDepOfInterleavingTrans(subset, dep_interleaving, allIndepSet);
				for(LPNTran tran : dep)
					subset.add(tran);
			}
			return subset;
	 ...
	 * @return
	 */
	private LPNTranSet getSubsetOfInterleavingTrans(LPNTranSet interleavingEnabledSet)
	{
		LPNTranSet set1 = new LPNTranSet();
		LPNTranSet set2 = new LPNTranSet();
		for(LPNTran tran : interleavingEnabledSet)
	 ...
	 * @return
	 */
	private LPNTranSet getDepOfInterleavingTrans(LPNTranSet subInterleavingSet, LPNTranSet dep_interleaving,
			                                     HashMap<LPNTran,HashSet<LPNTran>> allIndepSet)
	{
		//dependent on interleaving
		LPNTranSet dep_sub = new LPNTranSet();
		for(LPNTran tran : dep_interleaving)
		{
		...
		LPNTranSet dep_dep = new LPNTranSet();
		for(LPNTran tran : dep_interleaving)
		{
	...
	private LPNTranSet getEnableTranSet(LinkedList<LPNTran>[] enabledList)
	{
		LPNTranSet enableTranSet = new LPNTranSet();
